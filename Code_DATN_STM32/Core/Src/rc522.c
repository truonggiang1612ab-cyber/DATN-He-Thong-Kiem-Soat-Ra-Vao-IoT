...
static MFRC522_Status_t MFRC522_AnticollLevel(MFRC522_Name* MFRC522, uint8_t cascadeCmd, uint8_t* serNum)
{
    MFRC522_Status_t status;
    uint8_t i;
    uint8_t serNumCheck = 0;
    ...
    MFRC522_WriteRegister(MFRC522, MFRC522_REG_BIT_FRAMING, 0x00);
...
static MFRC522_Status_t MFRC522_SelectLevel(MFRC522_Name* MFRC522, uint8_t cascadeCmd, uint8_t* serNum, uint8_t* sak)
{
    uint8_t i;
    uint16_t recvBits;
    uint8_t buffer[9];
    ...
    MFRC522_Status_t status = MFRC522_ToCard(MFRC522, PCD_TRANSCEIVE, buffer, 9, buffer, &recvBits);
...
MFRC522_Status_t MFRC522_CheckUID(MFRC522_Name* MFRC522, RFID_UID_t* card)
{
    MFRC522_Status_t status;
    uint8_t atqa[10];
    uint8_t serNum[5];
    uint8_t sak = 0;
    uint8_t cascadeCmd = PICC_ANTICOLL_CL1;

    card->len = 0;
    memset(card->uid, 0, MFRC522_UID_MAX_LEN);

    status = MFRC522_Request(MFRC522, PICC_REQIDL, atqa);
    if(status != MI_OK)
    {
        return status;
    }

    status = MFRC522_AnticollLevel(MFRC522, cascadeCmd, serNum);
    if(status != MI_OK)
    {
        return status;
    }

    // Nếu không có cascade tag thì đây là thẻ UID 4 byte
    // Trả về luôn, giống logic thư viện cũ
    if(serNum[0] != PICC_CASCADE_TAG)
    {
        card->uid[0] = serNum[0];
        card->uid[1] = serNum[1];
        card->uid[2] = serNum[2];
        ...
        MFRC522_Halt(MFRC522);
        return MI_OK;
    }

    // Nếu có cascade tag thì đây là UID dài
    card->uid[card->len++] = serNum[1];
    card->uid[card->len++] = serNum[2];
    card->uid[card->len++] = serNum[3];

    status = MFRC522_SelectLevel(MFRC522, PICC_ANTICOLL_CL1, serNum, &sak);
    if(status != MI_OK)
    {
        MFRC522_Halt(MFRC522);
        return status;
    ...
    status = MFRC522_SelectLevel(MFRC522, PICC_ANTICOLL_CL2, serNum, &sak);
    if(status != MI_OK)
    {
        MFRC522_Halt(MFRC522);
        return status;
...
MFRC522_Status_t MFRC522_CompareUID(RFID_UID_t* a, RFID_UID_t* b)
{
    if(a->len != b->len) return MI_ERR;

    if(memcmp(a->uid, b->uid, a->len) == 0)
        return MI_OK;
