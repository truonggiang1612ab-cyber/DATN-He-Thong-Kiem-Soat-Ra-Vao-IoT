...
typedef enum
{
	DOOR_PHASE_OPENING, DOOR_PHASE_HOLDING_OPEN, DOOR_PHASE_CLOSING
} DoorPhase_t;
...
typedef enum
{
	LCD_ALIGNMENT_LEFT, LCD_ALIGNMENT_CENTER
} LcdAlignment_t;

typedef enum
{
	LCD_BEEP_NONE, LCD_BEEP_ONCE, LCD_BEEP_ERROR
} LcdBeepPattern_t;
...
typedef enum
{
	ACCESS_DIRECTION_EXITING, ACCESS_DIRECTION_ENTERING
} AccessDirection_t;
...
//  Quan ly phien xac thuc da yeu to mo cua và menu admin
...
uint32_t rfid_last_seen_tick = 0;
...
// Trang thai loi di va thoi gian tai luc phat hien huong di chuyen
...
// Xac thuc da yeu to mo cua và menu admin
...
// Lenh tu ESP32, Firebase va Blynk
...
// Gui 1 dong du lieu sang ESP32 theo dinh dang prefix + noi dung
void ESP_SendLine(char *prefix, char *text)
{
	char buffer[64];
...
// Gui dong thoi hai dong LCD sang ESP32 de dong bo giao dien Blynk
void ESP_SendLCD(char *line1, char *line2)
{
	ESP_SendLine("LCD1:", line1);
	ESP_SendLine("LCD2:", line2);
}
// Chi gui noi dung dong LCD thu hai sang ESP32
void ESP_SendLCD2(char *line2)
{
	ESP_SendLine("LCD2:", line2);
}
...
// Can giua noi dung và hien thi truc tiep cac so da nhap
void LCD_ShowCenteredInput(char *text)
{
...
// Hien thi mat khau da nhap bang dau sao va phan gach duoi
void LCD_ShowPasswordMask6(void)
{
...
// Ghi 1 mau ky tu tuy chinh vao bo nho CGRAM cua LCD
void LCD_CreateChar(uint8_t custom_char_slot, uint8_t custom_char_bitmap[])
{
...
// Xoa trang thai thao tac cu va dua he thong vao man hinh cua khoa
void HomeScreen(void)
{
...
// Tu huy thao tac va tro ve man hinh chinh khi nguoi dung khong thao tac qua lau
void UI_Timeout_Process(void)
{
...
// Kiem tra vi tri user co nam trong gioi han va dang chua du lieu hay khong
uint8_t UserDB_UserExists(uint8_t user_index)
{
	return user_index < MAX_USERS && USER_RECORDS[user_index].status != USER_STATUS_EMPTY;
}
// Kiem tra user co trang thai ACTIVE va duoc phep xac thuc mo cua hay khong
uint8_t UserDB_CanAuthenticate(uint8_t user_index)
{
	return user_index < MAX_USERS && USER_RECORDS[user_index].status == USER_STATUS_ACTIVE;
}
// Kiem tra user co trang thai PENDING hoac ACTIVE de duoc phep quan ly
uint8_t UserDB_CanManage(uint8_t user_index)
{
...
// Kiem tra chuoi co dung 6 ky tu so hay khong
uint8_t UserDB_IsSixDigitText(const char *text)
{
...
// Kiem tra mat khau thuong hoac khan cap da ton tai trong co so du lieu nguoi dung chua
uint8_t UserDB_PasswordExists(const char *password, uint8_t excluded_user_index)
{
...
int16_t FindUserByAnyPassword(const char *password, uint8_t *matched_emergency_password)
{
	...
	for (uint16_t i = 0U; i < MAX_USERS; i++)
	{
		...
		{
			if (matched_emergency_password != NULL)
			*matched_emergency_password = 1U;
			return (int16_t) i;
		}
	}
	return -1;
...
int16_t FindUserByFingerId(uint16_t fingerprint_template_id, uint8_t *matched_emergency_fingerprint)
{
	...
	if (matched_emergency_fingerprint != NULL)
	*matched_emergency_fingerprint = is_emergency_finger;
	return (int16_t) user_index;
}
// Dem lai tong so user khac trang thai EMPTY trong co so du lieu
void UserDB_Recount(void)
{
...
// Gui ACK ket qua xu ly kem command_id tuong ưng den ESP32
void UserDB_SendAck(const char *command_id, const char *ack_result)
{
	char ack_message[64];
...
// Hien thi ten, trang thai vao ra, tang va phong cua user vua xac thuc
void Access_ShowUserInfo(uint8_t user_index, char *access_action_text)
{
	char line1[17];
	char line2[17];
...
// Hien thi ngay va gio cua lan truy cập sau man hinh thong tin user
void Access_ShowTimeDate(void)
{
	char line1[17];
	char line2[17];
...
void Passage_Process(void)
{
	...
	case PASSAGE_A_BLOCKED_FIRST:
		...
		else if (!is_sensor_a_blocked && is_sensor_b_blocked) current_passage_state = PASSAGE_B_BLOCKED_AFTER_A;
		else if (!is_sensor_a_blocked) current_passage_state = PASSAGE_WAIT_SENSOR_B_AFTER_A;
		break;
	case PASSAGE_WAIT_SENSOR_B_AFTER_A:
		if (is_sensor_a_blocked && is_sensor_b_blocked) current_passage_state = PASSAGE_WAIT_BOTH_CLEAR;
		else if (is_sensor_a_blocked) current_passage_state = PASSAGE_A_BLOCKED_FIRST;
		else if (is_sensor_b_blocked) current_passage_state = PASSAGE_B_BLOCKED_AFTER_A;
		break;
	...
	case PASSAGE_B_BLOCKED_FIRST:
		if (is_sensor_a_blocked && is_sensor_b_blocked) current_passage_state = PASSAGE_WAIT_BOTH_CLEAR;
		else if (is_sensor_a_blocked && !is_sensor_b_blocked) current_passage_state = PASSAGE_A_BLOCKED_AFTER_B;
		else if (!is_sensor_b_blocked) current_passage_state = PASSAGE_WAIT_SENSOR_A_AFTER_B;
		break;
	case PASSAGE_WAIT_SENSOR_A_AFTER_B:
		if (is_sensor_a_blocked && is_sensor_b_blocked) current_passage_state = PASSAGE_WAIT_BOTH_CLEAR;
		else if (is_sensor_b_blocked) current_passage_state = PASSAGE_B_BLOCKED_FIRST;
		else if (is_sensor_a_blocked) current_passage_state = PASSAGE_A_BLOCKED_AFTER_B;
		break;
	...
	case PASSAGE_WAIT_BOTH_CLEAR:
		if (!is_sensor_a_blocked && !is_sensor_b_blocked) current_passage_state = PASSAGE_WAIT_FIRST_SENSOR;
		break;
	default:
		current_passage_state = PASSAGE_WAIT_BOTH_CLEAR;
		break;
...
// Hien thi 1 ban ghi nhat ky da luu tren LCD va gui thong tin sang ESP32
void AccessLog_Show(uint8_t log_index)
{
	char line1[17];
	char line2[17];
...
// Gui toan bo nhat ky dang luu sang ESP32 giua LOG START va LOG END
void ESP_SendAccessLogs(void)
{
	...
	for (uint8_t i = 0; i < access_log_count; i++)
	{
		...
		snprintf(access_log_tx_buffer, sizeof(access_log_tx_buffer),
				"%02u|%s|%s|%u|%u|%s|%s", i + 1,
...
// Xoa bo dem va so luong ky tu cua lan nhap hien tai
void Auth_ResetInput(void)
{
	input_length = 0;
...
// Sao luu du lieu the hien tai truoc khi bat dau chinh sua
void Auth_BeginCardEditSession(void)
{
	user_card_backup = myConfig.user_cards[credential_management_user_index];
	is_card_edit_active = 1;
}
// Khoi phuc du lieu the da sao luu khi qua trinh chinh sua bi huy
void Auth_CancelCardEditSession(void)
{
...
// Xoa toan bo du lieu cua phien xac thuc da yeu to hien tai
void AccessAuth_Reset(void)
{
...
// Dem so nhom khac nhau da duoc xac thuc trong mat na hien tai
uint8_t AccessAuth_CountMethods(uint8_t verified_method_mask)
{
...
// Khoi tao 1 phien xac thuc moi cho mo cua hoac menu admin
void AccessAuth_Start(AccessAuthTarget_t new_auth_target)
{
	AccessAuth_Reset();
...
// Hien thi tien do va yeu to con co the su dung trong phien xac thuc
void AccessAuth_ShowProgress(void)
{
	char line1[17];
	char line2[17];
...
// Gan phien xac thuc voi dung user va chi cho user admin truy cap chuc nang quan tri
uint8_t AccessAuth_LinkUser(uint8_t user_index)
{
...
// Ghi nhan 1 yeu to hop le, kiem tra cung user va hoan tat khi da du yeu cau
void AccessAuth_RecordVerifiedMethod(uint8_t user_index, uint8_t auth_method_mask_to_add, uint8_t is_verified_method_emergency)
{
...
void AccessAuth_ProcessPassword(const char *password)
{
	...
	if (access_auth_target == ACCESS_AUTH_TARGET_ADMIN_MENU)
	{
		if (!UserDB_CanAuthenticate(ADMIN_USER_INDEX) || strcmp(password, USER_RECORDS[ADMIN_USER_INDEX].user_password) != 0)
		{
			LCD_ShowMessage("LOI:NHAP SAI", "MAT KHAU ADMIN", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
			HomeScreen();
			return;
		}
		AccessAuth_RecordVerifiedMethod(ADMIN_USER_INDEX,
		AUTH_METHOD_PASSWORD_MASK, 0U);
		return;
	}
	user_index = FindUserByAnyPassword(password, &is_emergency_method);
	if (user_index >= 0)
	{
		AccessAuth_RecordVerifiedMethod((uint8_t) user_index,
		AUTH_METHOD_PASSWORD_MASK, is_emergency_method);
		return;
	}
...
void AccessAuth_ProcessCard(RFID_UID_t *card)
{
	...
	{
		AccessAuth_RecordVerifiedMethod((uint8_t) user_index,
		AUTH_METHOD_CARD_MASK, is_emergency_method);
		return;
	}
	LCD_ShowMessage("LOI:THE KHONG", "HOP LE", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
	HomeScreen();
}
// Cap quyen mo cua hoac truy cap menu admin sau khi du so yeu to yeu cau
void AccessAuth_Complete(void)
{
	uint8_t user_index;
...
// Chuyen sang buoc nhap USER_ID va xoa du lieu nhap truoc do
void Auth_ShowUserIdInput(void)
{
...
// Hien thi ten user kem dau ? de nguoi dung xac nhan dung ten
void Auth_ShowConfirmUser(void)
{
	char line2[17];
...
// Chuyen sang buoc nhap ma xac minh duoc cap tu he thong quan ly
void Auth_ShowActivationCode(void)
{
...
// Yeu cau user nhap mat khau hien tai truoc khi doi mat khau
void Auth_ShowCurrentUserPasswordInput(void)
{
...
void Auth_ShowNormalFinger(void)
{
	...
	Auth_ResetInput();
	expected_fingerprint_template_id =
	USER_RECORDS[credential_management_user_index].finger_id;
...
// Hien thi buoc them hoac bo qua the RFID thuong
void Auth_ShowNormalCard(void)
{
...
// Hien thi buoc them hoac bo qua the RFID khan cap
void Auth_ShowEmergencyCard(void)
{
...
// Khoi tao quy trinh quan ly van tay va bat dau tu buoc USER_ID
void Auth_StartFingerFlow(void)
{
...
// Khoi tao quy trinh quan ly the RFID va bat dau tu buoc USER_ID
void Auth_StartCardFlow(void)
{
...
// Khoi tao quy trinh doi mat khau user va bat dau tu buoc USER_ID
void Auth_StartPasswordFlow(void)
{
...
int16_t Auth_FindCardOwner(RFID_UID_t *card_uid, uint8_t include_emergency_card, uint8_t *matched_emergency_card)
{
	...
	for (uint16_t i = 0U; i < MAX_USERS; i++)
	{
		...
		{
			if (matched_emergency_card != NULL)
			*matched_emergency_card = 1U;
			return (int16_t) i;
		}
...
// Kiem tra the vua quet da duoc gan cho user nao chua
...
void RFID_Process(void)
{
	...
	{
		rfid_last_seen_tick = HAL_GetTick();
	...
	if (is_rfid_scan_handled)
	{
		if ((HAL_GetTick() - rfid_last_seen_tick) >= 500U) is_rfid_scan_handled = 0U;
	}
}
...
uint8_t Finger_DeleteTemplateIfExists(uint16_t fingerprint_template_id, uint8_t *was_template_deleted)
{
	...
	if (was_template_deleted != NULL)
	*was_template_deleted = 1U;
	return 1U;
}
...
// Cho nguoi dung dat ngon tay len cam bien trong thoi gian gioi han
uint8_t Auth_WaitFingerPlaced(uint32_t timeout_ms)
{
...
// Cho nguoi dung nhac ngon tay khoi cam bien trong thoi gian gioi han
uint8_t Auth_WaitFingerRemoved(uint32_t timeout_ms)
{
	uint32_t wait_start_tick = HAL_GetTick();
	while (HAL_GetTick() - wait_start_tick < timeout_ms)
...
// Thu hai mau van tay, tao mau hoan chinh va luu vao ID duoc chi dinh
...
// Quet van tay va ghi nhan sinh trac hoc cho phien mo cua hoac menu admin
void Finger_Process(void)
{
	int16_t user_index;
	...
	if (fingerprint_sensor_status == AS608_OK)
	{
		...
		if (user_index >= 0)
		{
			...
			AccessAuth_RecordVerifiedMethod((uint8_t) user_index,
			AUTH_METHOD_FINGER_MASK, is_emergency_method);
			return;
		}
		LCD_ShowMessage("LOI:VAN TAY", "KHONG DUOC CAP", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
		HomeScreen();
...
// Xu ly yeu cau mo cua, reset va xem nhat ky duoc gui tu Blynk qua ESP32
void ESP_Command_Process(void)
{
	if (is_esp_door_open_requested)
	{
		is_esp_door_open_requested = 0;
		if (current_system_state != STATE_HOME_SCREEN || is_access_auth_active || input_length > 0U) ESP_SendLine("CANH BAO:CO ", "NGUOI THAO TAC");
		else
		{
			Door_Open_Auto();
			current_system_state = STATE_DOOR_OPERATION;
...
int main(void) {
	...
	if (!was_config_loaded || myConfig.magic != CONFIG_MAGIC)
	{
		...
		if (!SaveDataToAT24C256((uint8_t*) &myConfig, sizeof(ConfigData_t))) Error_Handler();
	}
	access_log_count = myConfig.access_log_count;
	next_log_write_index = myConfig.access_log_index;
	if (access_log_count > MAX_ACCESS_LOG) access_log_count = 0;
	if (next_log_write_index >= MAX_ACCESS_LOG) next_log_write_index = 0;
	for (uint8_t i = 0; i < MAX_USERS; i++)
	{
		if (USER_RECORDS[i].status > USER_STATUS_ACTIVE) memset(&USER_RECORDS[i], 0, sizeof(USER_RECORDS[i]));
	...
	while (1) {
		if (!is_finger_sensor_ready && (current_system_state == STATE_HOME_SCREEN || current_system_state == STATE_ADMIN_AUTHENTICATION) && input_length == 0 && HAL_GetTick() - last_finger_reconnect_attempt_tick >= 5000U)
		{
			last_finger_reconnect_attempt_tick = HAL_GetTick();
			__HAL_UART_FLUSH_DRREGISTER(&huart2);
			if (AS608_CheckPassword(&finger_sensor) == AS608_OK) is_finger_sensor_ready = 1U;
		}
		Finger_Process();
		ESP_Command_Process();
		...
		case STATE_ADMIN_NEW_PASSWORD_INPUT:
			...
			else if (key == '#')
			{
				...
				if (!UserDB_CanAuthenticate(ADMIN_USER_INDEX))
				{
					LCD_ShowMessage("ADMIN KHONG", "HOAT DONG", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
					Admin_ShowSystemMenu();
					break;
				}
				if (strcmp(input_buffer, USER_RECORDS[ADMIN_USER_INDEX].duress_password) == 0 || UserDB_PasswordExists(input_buffer, ADMIN_USER_INDEX))
				{
					LCD_ShowMessage("PASS ADMIN TRUNG", "PASS DA TON TAI", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
					Admin_ShowPasswordInput();
					break;
				}
				UserProfileData_t admin_backup = USER_RECORDS[ADMIN_USER_INDEX];
				strcpy(USER_RECORDS[ADMIN_USER_INDEX].user_password, input_buffer);
				if (Storage_SaveUserProfile(ADMIN_USER_INDEX, &USER_RECORDS[ADMIN_USER_INDEX]))
				{
					LCD_ShowMessage("DOI PASS ADMIN", "THANH CONG", LCD_ALIGNMENT_LEFT, LCD_BEEP_ONCE, 1500);
				}
				else
				{
					USER_RECORDS[ADMIN_USER_INDEX] = admin_backup;
					if (!Storage_SaveUserProfile(ADMIN_USER_INDEX, &admin_backup))
					{
						ESP_SendLine("STORAGE:", "ADMIN_ROLLBACK_ERROR");
					}
					LCD_ShowMessage("LUU DU LIEU", "THAT BAI",
							LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
				}
				Admin_ShowSystemMenu();
				break;
			}
		...
		case STATE_ADMIN_DELETE_ALL_FINGERPRINTS:
		{
			...
			uint16_t admin_template_id =
			USER_RECORDS[ADMIN_USER_INDEX].finger_id;
		...
		case STATE_ACTIVATION_CODE_INPUT:
			...
			if (key == '#')
			{
				...
				if (strcmp(input_buffer, USER_RECORDS[credential_management_user_index].activation_code) != 0)
				{
		...
		case STATE_AUTH_METHOD_DELETE_CONFIRM:
			...
			if (key == '#')
			{
				...
				else if (current_credential_management_flow == CREDENTIAL_MANAGEMENT_FLOW_FINGERPRINT)
				{
					...
					else if (!was_any_template_deleted) LCD_ShowMessage("VAN TAY CAN XOA", "KHONG TON TAI", LCD_ALIGNMENT_LEFT, LCD_BEEP_ERROR, 1500);
					else LCD_ShowMessage("XOA VAN TAY", "THANH CONG", LCD_ALIGNMENT_LEFT, LCD_BEEP_ONCE, 1500);
				}
				else
				{
					HomeScreen();
...
static void MX_GPIO_Init(void) {
	...
	COL_1_Pin | COL_2_Pin | BUZZER_Pin | LED_OPEN_Pin | LED_LOCK_Pin,
			GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(GPIOB,
			COL_3_Pin | COL_4_Pin | SDA_Pin | LCD_D4_Pin | LCD_D5_Pin
					| LCD_D6_Pin | LCD_D7_Pin | TRIG_Pin | LCD_RS_Pin,
			GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pin = COL_3_Pin | COL_4_Pin | SDA_Pin | LCD_D4_Pin
			| LCD_D5_Pin | LCD_D6_Pin | LCD_D7_Pin | TRIG_Pin | LCD_RS_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
